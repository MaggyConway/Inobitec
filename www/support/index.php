...
$APPLICATION->SetPageProperty("description", "Компания Инобитек обеспечивает полный цикл услуг заказной разработки программного обеспечения в медицинской сфере");
$APPLICATION->SetPageProperty("keywords", "Разработка, программное обеспечение, здравоохранение, медицина, заказ, на заказ");
$APPLICATION->SetPageProperty("title", "Инобитек / Разработка программного обеспечения для медицины на заказной основе");
...
<div class="support-search">
  <div class="wr">
    <form action="/search/" method="GET">
      ...
      <button class="btn btn--light-blue-outline support-search__button" type="submit"><span>Найти</span>
...
<div class="support-form" id="support-form">
  <div class="wr">
    <div class="support-form__title">
      ...
      <h2>Задать вопрос или сообщить о проблеме</h2>
    </div>
    <div class="support-form__toggle">
      <div class="support-form__radio">
        ...
        <label for="question">Задать вопрос</label>
      </div>
      <div class="support-form__radio">
        <input class="visually-hidden" type="radio" name="message-type" id="issue" value="issue">
        <label for="issue">Сообщить о проблеме</label>
      </div>
    </div>
...
grecaptcha.ready(function() {
    grecaptcha.execute('6LdNy7wUAAAAAPeRFWzbD3GIijmFnH-dJ_oNE8YD', {action: 'homepage'}).then(function(token) {
    	$('<input type="hidden" name="recaptcha_response">').val(token).prependTo($('form[name="QUESTION_FORM_RUS"]'));
        $('<input type="hidden" name="recaptcha_response">').val(token).prependTo($('form[name="TROUBLE_FORM_RUS"]'));
    });
});
</script>
