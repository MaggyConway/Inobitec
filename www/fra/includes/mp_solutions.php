    ...
    if($preItems->result->num_rows && $preItems->result->num_rows > 0){
      ...
      while($arItem = $preItems->GetNext()){
			...
			$in_array[$arItem["ID"]] = $arItem["ID"];
				...
				echo '<pre>';
				print_r($arItem['PROPERTY_INSERT_GOODS_VALUE']);
				echo '</pre>';
                if($arItem['PROPERTY_INSERT_GOODS_VALUE']){
                  $allInsertItems = array();
                  $insertItems = CIBlockElement::GetList(
                      array('sort' => 'asc', 'active_from' => 'desc'),
                      array('IBLOCK_ID' => 104, "ACTIVE" => "Y", "IBLOCK_SECTION_ID" => $arItem['PROPERTY_INSERT_GOODS_VALUE']),
                      false,
                      false,
			...
			// $res = CIBlockElement::GetList(
				// array("SORT"=>"ASC"), 
				// array('IBLOCK_ID' => 108, "ACTIVE" => "Y", "ID" => $arItem["ID"]), 
				// false, 
				// false, 
				// array("ID", "IBLOCK_ID")
			// );
			// while ($arr = $insertItems->GetNextElement()) {
				// $fields = $arr->GetFields();
				// $arrFilesPDF[$fields['ID']] = $arr->GetProperty('INSERT_GOODS');
				// echo '<pre>';
				// print_r($arrFilesPDF);
				// echo '</pre>';
			// }
...
<?
			// $res = CIBlockElement::GetList(
				// array("SORT"=>"ASC"), 
				// array("IBLOCK_ID" => 67, "ACTIVE" => "Y", "ID" => $arItem["ID"]), 
				// false, 
				// false, 
				// array("ID", "IBLOCK_ID")
			// );
			// while ($arr = $res->GetNextElement()) {
				// $fields = $arr->GetFields();
				// $arrFiles[$fields['ID']] = $arr->GetProperty('INSERT_GOODS');
			// }
			// foreach($arrFiles as $id => $item){
				// foreach($item['VALUE'] as $num => $v){
					// $manual = CFile::GetFileArray($v);
					// if($manual){
						// print_r($manual);
					// }
				// }
