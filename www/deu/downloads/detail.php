...
$APPLICATION->SetPageProperty("title", "Inobitec DICOM-viewer und server fur OS: Windows, Mac OS, Linux");
$APPLICATION->SetTitle("Versionen zum Herunterladen");
$no_f_links = true;
        ...
        if (CModule::IncludeModule('iblock')) {
            if ($arItem['PROPERTY_WIN_DISTR_VALUE']) {
                ...
                if ($сache->initCache(36000, 'downloads_win_' . $arItem['ID'] . '_' . SITE_ID)) {
                    $winItems = $сache->getVars();
                }elseif ($сache->startDataCache()) {
                    $preFileDistribs = CIBlockElement::GetList(
                        ['sort' => 'asc', 'active_from' => 'desc'],
                        ['IBLOCK_CODE' => 'distribs_s1', "ACTIVE" => "Y", "=ID" => $arItem['PROPERTY_WIN_DISTR_VALUE']],
                        false,
                        [],
                        ['ID', 'NAME', 'PREVIEW_TEXT', 'DETAIL_TEXT', 'PROPERTY_SELECT_FILES']
                    );
                    while ($distrib = $preFileDistribs->GetNext()) {
                        ...
                        foreach ($distrib['PROPERTY_SELECT_FILES_VALUE'] as $key => $file) {
                            $preFilesBlock = CIBlockElement::GetList(
                                ['sort' => 'asc', 'active_from' => 'desc'],
                                ['IBLOCK_CODE' => 'files', "ACTIVE" => "Y", "=ID" => $file],
                                false,
                                [],
                                ['ID', 'NAME', 'PROPERTY_FILE', 'PROPERTY_LESS_FILENAME', 'PROPERTY_ABOUT']
                            );
                    ...
                    $сache->endDataCache($winItems);
                } ?>
				<div class="download__description">
					<div
						class="download__description-logo <?=($arItem['PROPERTY_NEW_DESIGN_VALUE'] == 'viewer') ? 'download__description-logo-viewer' : ''?>">
						<img src="/verstka_new/buy_p/img/inobitec-web/inobitec-web-description-logo-2.svg" alt="">
					</div>
					<div
						class="download__description-case <?=($arItem['PROPERTY_NEW_DESIGN_VALUE']) ? 'download__description-case-' . $arItem['PROPERTY_NEW_DESIGN_VALUE'] : ''?>">
						<p class="text-bold text-big"><?=$result["PROPERTY_WIN_TITLE_VALUE"]?></p>
                        <?
                        foreach ($winItems as $i => $winFile) { ?>
							<a href="<?=$winFile['PROPERTY_FILE_VALUE']?>"
							   target="_blank"><?=$winFile['PROPERTY_LESS_FILENAME_VALUE']?></a>
                        <?
                        } ?>
					</div>
				</div>
        ...
        if (CModule::IncludeModule('iblock')) {
            if ($arItem['PROPERTY_MAC_DISTR_VALUE']) {
                ...
                if ($сache->initCache(36000, 'downloads_mac_' . $arItem['ID'] . '_' . SITE_ID)) {
                    $macItems = $сache->getVars();
                }elseif ($сache->startDataCache()) {
                    $preFileDistribs = CIBlockElement::GetList(
                        ['sort' => 'asc', 'active_from' => 'desc'],
                        ['IBLOCK_CODE' => 'distribs_s1', "ACTIVE" => "Y", "=ID" => $arItem['PROPERTY_MAC_DISTR_VALUE']],
                        false,
                        [],
                        ['ID', 'NAME', 'PREVIEW_TEXT', 'DETAIL_TEXT', 'PROPERTY_SELECT_FILES']
                    );
				...
				<div class="download__description">
					...
					<div
                        ...
                        foreach ($macItems as $i => $macFile) { ?>
							<a href="<?=$macFile['PROPERTY_FILE_VALUE']?>"
							   target="_blank"><?=$macFile['PROPERTY_LESS_FILENAME_VALUE']?></a>
                            <?
                        } ?>
					</div>
				</div>
        ...
        if (CModule::IncludeModule('iblock')) {
            ...
            if ($arItem['PROPERTY_LINUX_DISTR_VALUE']) {
                ...
                if ($сache->initCache(36000, 'downloads_linux_' . $arItem['ID'] . '_' . SITE_ID)) {
                    $linuxItems = $сache->getVars();
                }elseif ($сache->startDataCache()) {
                    $preFileDistribs = CIBlockElement::GetList(
                        ['sort' => 'asc', 'active_from' => 'desc'],
                        [
                            'IBLOCK_CODE' => 'distribs_s1',
                            "ACTIVE"      => "Y",
                            "=ID"         => $arItem['PROPERTY_LINUX_DISTR_VALUE'],
                        ],
                        false,
                        [],
                        ['ID', 'NAME', 'PREVIEW_TEXT', 'DETAIL_TEXT', 'PROPERTY_SELECT_FILES']
				...
				<div class="download__description">
					...
					<div
                        ...
                        foreach ($linuxItems as $i => $linuxFile) { ?>
							<a href="<?=$linuxFile['PROPERTY_FILE_VALUE']?>"
							   target="_blank"><?=$linuxFile['PROPERTY_LESS_FILENAME_VALUE']?></a>
                        <?
                        } ?>
					</div>
				</div>
