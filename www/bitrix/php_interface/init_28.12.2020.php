...
function basketInfo(){
  ...
  while ($arItems = $dbBasketItems->GetNext()) {
    ...
    if(checkIfLicense($obGoods)){
	  ...
	  $item["name"] = $tmp_name; //(SITE_ID_FOR_BASKET == "s1") ? $obGoods["NAME"] : $obGoods["PROPERTY_NAME_EN_VALUE"];
      $item["cartitemId"] = $arItems["ID"];
    ...
    foreach($licView as $key => $lic){
      foreach($lic["PROPS"] as $prop){
        if($prop["CODE"] == "UNIQUE_CODE"){
          ...
          $licView[$key]["NAME"] .= (SITE_ID == "s2")?"#br#".$obProduct["PROPERTY_NAME_EN_VALUE"]:"#br#".$obProduct["NAME"];
        }
    ...
    foreach($licServ as $key => $lic){
      foreach($lic["PROPS"] as $prop){
        if($prop["CODE"] == "UNIQUE_CODE"){
          ...
          $obProduct = $resProduct->getNext();
          $licServ[$key]["NAME"] .= (SITE_ID == "s2")?"#br#".$obProduct["PROPERTY_NAME_EN_VALUE"]:"#br#".$obProduct["NAME"];
        }
      }
    }
