...
function dicomviewerDefaulData($sectionID, $iBlockID){
  $resModules = CIBlockElement::GetList(
			...
			false,
			false,
			array('ID', 'NAME', 'DETAIL_TEXT', 'CATALOG_GROUP_1', "PROPERTY_NAME_EN", "PROPERTY_DESCRIPTION_EN")
		);
  $arrModules = array();
  while($obModule = $resModules->getNext()){
    $item = array();
    $item["name"] = (SITE_ID_FOR_BASKET == "s2") ? $obModule["PROPERTY_NAME_EN_VALUE"] : $obModule["NAME"];
  ...
  $resGoods = CIBlockElement::GetList(
      ...
      false,
      false,
      array('ID', 'NAME', 'DETAIL_TEXT', 'CATALOG_GROUP_1', "PROPERTY_LIC_TYPE", "PROPERTY_SHORT_NAME", "DETAIL_PICTURE", "PROPERTY_PREVIEW_TEXT_EN", "PROPERTY_NAME_EN", "PROPERTY_SHORT_NAME_EN", "PROPERTY_IMAGE_EN", "PROPERTY_IMAGE_RU")
  );
  $arrGoods = array();
...
function dicomserverDefaulData($sectionID, $iBlockID){
  ...
  while($obGoods = $resGoods->getNext()){
    ...
    while($obOffer = $resOffers->getNext()){
      ...
      $offer["name"] = (SITE_ID_FOR_BASKET == "s2") ? $obOffer["PROPERTY_NAME_EN_VALUE"] : $obOffer["NAME"];
      $offer["price"] = number_format($obOffer["CATALOG_PRICE_1"], 0, '', ' ');
...
function basketInfo(){
  ...
  while($obModule = $resModules->getNext()){
    $item = array();
    $item["name"] = (SITE_ID_FOR_BASKET == "s1") ? $obModule["NAME"] : $obModule["PROPERTY_NAME_EN_VALUE"];
    $item["price"] = number_format($obModule["CATALOG_PRICE_1"], 0, '', ' ');
    $item["inbasket"] = false;
    $item["quantity"] = 1;
  ...
  while ($arItems = $dbBasketItems->GetNext()) {
    ...
    if(checkIfLicense($obGoods)){
      ...
      if($imageLogo && isset($imageLogo["SRC"]))
        $item["img"] = $imageLogo["SRC"];
      $item["price"] = number_format($arItems["PRICE"], 0, '', ' ');
      $item["name"] = (SITE_ID_FOR_BASKET == "s1") ? $obGoods["NAME"] : $obGoods["PROPERTY_NAME_EN_VALUE"];
      $item["cartitemId"] = $arItems["ID"];
      $item["inbasket"] = true;
      $item["quantity"] = $arItems["QUANTITY"];
    ...
    foreach($licServ as $keyLS => $valLS){
      if(count($serverArr) > 0){
        ...
        if(is_array($arrParentGood))
          ...
          if(SITE_ID == "s2" && $arrGood["PROPERTY_NAME_EN_VALUE"]){
...
function prepareLicenseForUpdateToAPI($orderID){
  ...
  foreach($allLicUpdateArr as $licData){
    updateLicenseToAPIParseData($licData, $inobitecLicense);
    sleep(1);
  }
  
}

function updateLicenseToAPIParseData($obUpdateLic, $inobitecLicense){
  ...
  foreach($arResult["ITEMS"] as $item){
    ...
    foreach($item["PROPS"] as $arrProp){
      if($arrProp["CODE"] == "YEARS"){
        $str .=  ". ";
        $str .= (SITE_ID == 's2') ? "Increase the subscription validity term (number of years)" : $arrProp["NAME"];
        $str .= ": " . $arrProp["VALUE"];
      }
    }
    $str .= " - " . $item["QUANTITY"];
    $str .= (SITE_ID == 's2') ? " pcs. x " : " шт. x ";
    $str .= number_format($item["PRICE"],0,""," ") . " &#8381;<br>";
