...
<div class="section">
	...
	<div class="bx_section">
		<h4 style="border-bottom:none;"><?=GetMessage("SOA_TEMPL_PAY_SYSTEM")?></h4>
        <hr>
        <div class='radio'>
          <div class='radio_box'>
		<?
        ...
        foreach($arResult["PAY_SYSTEM"] as $key => $checkSystem){
          if((SITE_ID == 's2' || SITE_ID == 's3' ) && ($checkSystem['ID'] == 3 || $checkSystem['ID'] == 4))
            unset($arResult["PAY_SYSTEM"][$key]);
          if( (isset($arResult['PERSON_TYPE'][2]['CHECKED']) && $arResult['PERSON_TYPE'][2]['CHECKED']=="Y") && SITE_ID == 's1' && $checkSystem['ID'] == 2)
            unset($arResult["PAY_SYSTEM"][$key]);
        }
        if(count($arResult["PAY_SYSTEM"]) == 1){
          foreach($arResult["PAY_SYSTEM"] as $key => $checkSystem){
            if($checkSystem["CHECKED"] != 'Y')
		...
		foreach($arResult["PAY_SYSTEM"] as $arPaySystem)
		{
			if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0)
			{
				...
				{
					?>
					<div class="bx_block w100 vertical test2">
						<div class="bx_element">
							<input type="radio"
                            ...
                            <label class="in_radio <?=($arPaySystem["CHECKED"]=="Y" && !($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && $arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y"))?"checked":""?> ">
                                <div class="in_radio_button"></div>
                                <div class="in_radio_text">
                                  <?if ($arParams["SHOW_PAYMENT_SERVICES_NAMES"] != "N"):?>
                                      <?=GetMessage('CUSTOM_ORDER_PAYMENT_TYPE_NAME_'.$arPaySystem["ID"]);?>
                                  <?else:?>
                                      <?="&nbsp;"?>
                                  <?endif;?>
                                </div>
				...
				?>
					<div class="bx_block w100 vertical test2">
						<div class="bx_element">
                            ...
                            <label class="in_radio <?=($arPaySystem["CHECKED"]=="Y" && !($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && $arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y"))?"checked":""?> " onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
                                <div class="in_radio_button"></div>
                                <div class="in_radio_text">
                                  <?if ($arParams["SHOW_PAYMENT_SERVICES_NAMES"] != "N"):?>
                                      <?=GetMessage('CUSTOM_ORDER_PAYMENT_TYPE_NAME_'.$arPaySystem["ID"]);?>
			...
			if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) == 0 && intval($arPaySystem["PRICE"]) == 0)
			{
				...
				{
					?>
					<div class="bx_block horizontal test4">
						<div class="bx_element">
                            ...
                            <?
                            /*
							<label style='display:none;' for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
								<?
								if (count($arPaySystem["PSA_LOGOTIP"]) > 0):
									$imgUrl = $arPaySystem["PSA_LOGOTIP"]["SRC"];
								else:
