    ...
    <div class="popup-main">
        ...
        <form name="form_auth" method="post" target="_top" action="<?=($arParams['POPUP'] === 'Y' ? $templateFolder . '/authAjax.php' : $arResult['AUTH_URL']);?>">
            ...
            <label class="input <?if ($loginError): ?>error<?endif;?>">
                <input name="USER_LOGIN" placeholder="<?=GetMessage('AUTH_LOGIN');?>" type="text" value="<?= ($_POST['USER_LOGIN'])? $_POST['USER_LOGIN'] : $arResult["LAST_LOGIN"]?>" >
        ...
        <?if ($arParams["POPUP"] === "Y"): ?><a href="javascript:;" onclick="closePopup(document.getElementById('authorization'));Popup('pass');"><?else:?><a href="<?=$arResult['AUTH_FORGOT_PASSWORD_URL'];?>"><?endif;?><?=GetMessage('AUTH_FORGOT_PASSWORD_2');?></a>
    </div>
</div>
