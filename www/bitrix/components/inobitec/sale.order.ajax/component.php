...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	...
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
        ...
        if($isAjaxRequest)
          $newItems = customCatalog::updateBasketCurrency(SITE_ID);
		addToLog("обновили товар");
		...
		if (empty($arResult["BASKET_ITEMS"]) || !is_array($arResult["BASKET_ITEMS"]))
		{
			...
			else
			{
				if (isset($_REQUEST['json']) && $_REQUEST['json'] == "Y")
				{
                    ...
                    //print_r("<br>");
                    //print_r($arResult["PATH_TO_BASKET"]);
			...
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
					...
					if(strlen($arUserResult["PAYER_NAME"]) > 0)
					{
						...
						$NEW_NAME = $arUserResult["PAYER_NAME"];
...
*/
                    ...
                    if ($arUserLogin = $dbUserLogin->Fetch())
                    {
                        $arResult["ERROR"][] = GetMessage("STOF_ERROR_EMAIL_EXIST");
                    }
                    $arFilterEmail = Array(
                      "EMAIL"=>$NEW_LOGIN
                    );
                    $dbUsersEmail = CUser::GetList(($by = "NAME"), ($order = "desc"), $arFilterEmail);
                    if ($arUserEmail = $dbUsersEmail->Fetch())
                    {
                        $arResult["ERROR"][] = GetMessage("STOF_ERROR_EMAIL_EXIST");
                    }
                    ...
                    if(empty($arResult["ERROR"])){
                      ...
                      $setUsertype = 0;
                      switch($arUserResult["PERSON_TYPE_ID"]){
                        case 1:
                          $setUsertype = 2;
                          break;
                        case 3:
                      ...
                      $userFields = Array(
                          ...
                          "ACTIVE" => "Y",
                          "LID" => SITE_ID,
                          "PERSONAL_NOTES" => "check",
                          "UF_USER_TYPE" => $setUsertype,
                          );
                      $NEW_PASSWORD = $NEW_PASSWORD_CONFIRM = 0;
                      foreach($arResult["ORDER_PROP"]["PRINT"] as $propInfo){
                        $OrderCode = (isset($arResult["ORDER_PROP"]["USER_PROPS_N"][$propInfo["ID"]])) ? $arResult["ORDER_PROP"]["USER_PROPS_N"][$propInfo["ID"]]["CODE"] : $arResult["ORDER_PROP"]["USER_PROPS_Y"][$propInfo["ID"]]["CODE"];
                        if($OrderCode == "PASS" || $OrderCode == "PASS_CONF"){
                          $NEW_PASSWORD = $NEW_PASSWORD_CONFIRM = $propInfo["VALUE"];
                          
                        }
                        
                        $UserFieldCode = componentGetUserPropByOrderProp($OrderCode);
                        //addToLog(print_r($propInfo["ID"]."--".$propInfo["NAME"]."--".$propInfo["VALUE"]."--".$OrderCode."--".$UserFieldCode, true));
                        if($UserFieldCode){
                          $val = $propInfo["VALUE"];
                          if($OrderCode == "WORK_PHONE"){
                            if(SITE_ID == "s1"){
                              if(substr($val, 0, 1) != "+"){
                                $val = "+7".preg_replace('~[^0-9+]+~','',$val);
                              }
                            }else{
                              if(substr($val, 0, 1) != "+"){
                                $val = "+".$val;
                          ...
                          if($OrderCode == "EVIDENCE"){
                            foreach($arResult["ORDER_PROP"]["USER_PROPS_N"][$propInfo["ID"]]["VARIANTS"] as $variant){
                              if($variant["NAME"] == $propInfo["VALUE"]){
                          ...
                          $userFields[$UserFieldCode] = htmlspecialchars_decode($val);
                        }
                        
                      }
                      
                      if(strlen($NEW_PASSWORD) < $password_min_length)
                        $arResult["ERROR"][] = GetMessage("STOF_ERROR_SHORT_PASS");
                      
                      if(!preg_match("/^[A-Za-z0-9]+$/",$NEW_PASSWORD))
                        $arResult["ERROR"][] = GetMessage("STOF_ERROR_PASS_SYMBOLS");
                      
                      if(empty($arResult["ERROR"])){
                        $userFields["PASSWORD"] = $NEW_PASSWORD;
                        $userFields["CONFIRM_PASSWORD"] = $NEW_PASSWORD_CONFIRM;

                        $user = new CUser;

                        //addToLog(print_r($arResult["ORDER_PROP"], true));
                        //addToLog(print_r($userFields, true));
                        $arAuthResult = $user->Add($userFields);
                        ...
                        else
                        {
                            ...
                            {
                                if($arParams["SEND_NEW_USER_NOTIFY"] == "Y")
                                    CUser::SendUserInfo($USER->GetID(), SITE_ID, $NEW_PASSWORD, true);
                                addUserToAPI($USER->GetID());
                            }
                            else
                            {
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
                ...
                orderToLicense($arResult["ORDER_ID"]);
				...
				if ($arResult["ORDER_ID"] > 0 && empty($arResult["ERROR"]))
				{
                    ...
                    $resClearBasketRu = CSaleBasket::GetList(array(), array(
                              'FUSER_ID' => CSaleBasket::GetBasketUserID(),
                              'LID' => 's1',
                              'ORDER_ID' => 'null',
                              ));
                    while ($rowClearBasketRu = $resClearBasketRu->fetch()) {
                       CSaleBasket::Delete($rowClearBasketRu['ID']);
                    }
                    
                    $resClearBasketEn = CSaleBasket::GetList(array(), array(
                              'FUSER_ID' => CSaleBasket::GetBasketUserID(),
                              'LID' => 's1',
                              'ORDER_ID' => 'null',
                              ));
                    while ($rowClearBasketEn = $resClearBasketEn->fetch()) {
                       CSaleBasket::Delete($rowClearBasketEn['ID']);
                    }
