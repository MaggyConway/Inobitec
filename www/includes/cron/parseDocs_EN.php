...
$fileStr = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/includes/cron/html_default/Viewer_default.icnt");
//ini_set('display_errors', 1);
//error_reporting(E_ALL);

define('EN_DOCS_IBLOCK_ID', 38);
define('DEFAULT_IMG_PATTH', "/includes/cron/html_default");
        ...
        foreach($node->childNodes as $nodeContent){
          if ($nodeContent->nodeType == XML_ELEMENT_NODE){
            if($nodeContent->getAttribute('class')  == 'Content'){
              if(!file_exists(__DIR__ . "/html_default/" . $nodeContent->firstChild->getAttribute('id'))){
...
foreach($newSections as $section){
  ...
  foreach ( $src as $s ) {
    ...
    if(isset($arrLinksToId[$arrHref[0]])){
      $firstPart = "/eng" . getUrlByCodeID($arrLinksToId[$arrHref[0]], $newItems, $newSections);
      unset($arrHref[0]);
      if(count($arrHref > 0)){
...
function getFileData($filename){
  ...
  $fileStr = file_get_contents(__DIR__ . "/html_default/" . $filename);
...
function getDBSections(){
  $oldSections = array();
  $arFilter = array('IBLOCK_ID' => EN_DOCS_IBLOCK_ID);
...
function getDBItems(){
  ...
  $arFilter = Array("IBLOCK_ID"=>IntVal(EN_DOCS_IBLOCK_ID));
...
function updateSection($item, $num, $ID){
  $bs = new CIBlockSection;
  $preName = strstr($item["docUrl"],'Viewer_defaultch') ? "Chapter " : '';
