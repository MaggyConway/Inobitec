...
$fileStr = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/includes/cron/ru_docs_PACS/PACSServer_ru.icnt");
//ini_set('display_errors', 1);
//error_reporting(E_ALL);

define('RU_DOCS_IBLOCK_ID', 1);
define('DEFAULT_IMG_PATTH', "/includes/cron/ru_docs_PACS");
        ...
        foreach($node->childNodes as $nodeContent){
          if ($nodeContent->nodeType == XML_ELEMENT_NODE){
            if($nodeContent->getAttribute('class')  == 'Content'){
              if(!file_exists(__DIR__ . "/ru_docs_PACS/" . $nodeContent->firstChild->getAttribute('id'))){
...
function getFileData($filename){
  ...
  $fileStr = file_get_contents(__DIR__ . "/ru_docs_PACS/" . $filename);
...
function updateSection($item, $num, $ID){
  $bs = new CIBlockSection;
  $preName = strstr($item["docUrl"],'PACSServer_ruch') ? "Глава " : '';
