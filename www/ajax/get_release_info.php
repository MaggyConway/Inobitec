...
<?
	if($_POST['ITEM_ID'] && $_POST['ITEM_ID'] != ''){
		...
		if($сache -> initCache(36000, 'product-features__'.$_POST['ITEM_ID'].''.$_POST['SITE_ID'])) 
		{ 
		...
		{ 
			if(CModule::IncludeModule('iblock')){
				$preInfo = CIBlockElement::GetList(
					array('sort' => 'asc'),
					array('IBLOCK_CODE' => 'distribs_'.$_POST['SITE_ID'], 'ID' => $_POST['ITEM_ID']),
					false,
					array('nTopCount' => 1),
					array('ID', 'NAME', 'DETAIL_TEXT', 'PREVIEW_PICTURE')
				);
