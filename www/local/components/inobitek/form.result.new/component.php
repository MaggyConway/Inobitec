...
/*************************************************************************************************/
	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
        ...
        if(isset($_POST['CUSTOM_AJAX']) && $_POST['CUSTOM_AJAX'] == 'Y'){
          $arParams['USE_EXTENDED_ERRORS'] = "Y";
        }
		...
		if (($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) && (strlen($_REQUEST["web_form_submit"])>0 || strlen($_REQUEST["web_form_apply"])>0))
		{
			...
			// check errors
			$arResult["FORM_ERRORS"] = CForm::Check($arParams["WEB_FORM_ID"], $arResult["arrVALUES"], false, "Y", "Y");
			$captchaCheck = checkGoogleRecaptcha();

            //$captchaCheck = $recaptcha_success;
            //$captchaCheck = true;
            //$arResult["TEST"]['Step'][] = "CHECK CAPTCHA";
			//file_put_contents(__DIR__ . "/log.txt", print_r($arResult["FORM_ERRORS"], true), FILE_APPEND);
			//file_put_contents(__DIR__ . "/log.txt", print_r($captchaCheck, true), FILE_APPEND);
			...
			if (!$captchaCheck) 
			{
				if ($arParams['USE_EXTENDED_ERRORS'] != 'Y') 
				{
					...
					{
                        $arResult['FORM_ERRORS'] .= '<br>';
					}
                    $arResult['FORM_ERRORS'] = GetMessage('FORM_NOTE_RECAPTCHA');
				}else{
                  $arResult['FORM_ERRORS']['UNCORRECT_CAPTCHA'] = GetMessage('FORM_NOTE_RECAPTCHA');	
                }
                $arResult['UNCORRECT_CAPTCHA'] = "Y";
			}
            
            $arResult["TEST"]['Step'][] = "CHECK ERRORS";
			...
			{
				// check user session
                $arResult["TEST"]['Step'][] = "CHECK SESSION ID";
                $arResult["TEST"]['sesid'] = bitrix_sessid();
